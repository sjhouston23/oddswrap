...
_BASE_URL = "https://sportsbook-nash.draftkings.com/api/sportscontent/dkusnj/v1/leagues/{league_id}/categories/{category_id}"


def _parse_american(s: str) -> Optional[int]:
    try:
...
class DraftKingsAdapter(BookAdapter):
    ...
    def fetch_moneylines(self, sport: Sport) -> List[Game]:
        ...
        games: List[Game] = []
        for mid, (eid, sels) in grouped.items():
            ev = events.get(eid)
            if not ev:
                continue
            ...
            games.append(Game(
                sport=sport.value, home_team=home_raw, away_team=away_raw,
                start_time=ev.get("startDate"), game_id=str(eid),
